...
const SHACLcheck = require('./consolid/shacl-check') 
...
class ACLChecker {
  ...
  async can (user, mode, DynCerts) {
    ...
    let accessDenied = aclCheck.accessDenied(acl.graph, resource, directory, aclFile, agent, modes, agentOrigin, trustedOrigins, originTrustedModes, DynCerts)
    
    // added shacl checking functionality
    const anyDynamic = acl.graph.holds(null, RDF('type'), CS('DynamicRule'), aclFile)
    if (accessDenied && DynCerts && anyDynamic) {
      try {
        accessDenied = await SHACLcheck.shaclDenied(acl.graph, aclFile, modes, agent, DynCerts)
      } catch (error) {
        accessDenied = true
      }
    }
