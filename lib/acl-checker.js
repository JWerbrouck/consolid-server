...
const {shaclDenied, extractCertificate, verifyUser} = require('./consolid') 
...
class ACLChecker {
  ...
  async can (user, mode, req) {
    if (!user || user === undefined) {
      if (req) {
        try {
          const verifiedUser = await verifyUser(req)
          user = verifiedUser.webId
        } catch (error) {
        }
      }
    }
    ...
    // added shacl checking functionality
    const anyDynamic = acl.graph.holds(null, RDF('type'), CS('DynamicRule'), aclFile)
    if (accessDenied && anyDynamic && req !== undefined) {
      try {
        const DynCerts = await extractCertificate(req)
        if (DynCerts) {
          accessDenied = await shaclDenied(acl.graph, aclFile, modes, agent, DynCerts)
        }
      } catch (error) {
        accessDenied = true
      }
  ...
  async getError (user, mode, req) {
    const cacheKey = `${mode}-${user}`
    this.aclCached[cacheKey] = this.aclCached[cacheKey] || await this.can(user, mode, req)
