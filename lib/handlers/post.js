...
async function handler (req, res, next) {
  ...
  function multi () {
    ...
    busboy.on('file', async function (fieldname, file, filename, encoding, mimetype) {
      ...
      let p
      if (ldp.resourceMapper._rootPath.startsWith('http')) {
        p = ldp.resourceMapper._rootPath + path.join(containerPath, filename)
      } else {
        p = '/' + req.hostname + path.join(containerPath, filename)
      }

      const { url: putUrl } = await ldp.resourceMapper.mapFileToUrl(
        { path: p, hostname: req.hostname })
      try {

        await ldp.put(putUrl, file, 'text/turtle')
      } catch (err) {
