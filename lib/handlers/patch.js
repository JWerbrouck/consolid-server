...
const PATCH_PARSERS = {
  ...
  'application/json': require('./patch/sparql-update-parser.js'),
  'application/x-www-form-urlencoded': require('./patch/sparql-update-parser.js')
}
...
async function checkPermission(request, patchObject) {
  ...
  let allowed
  try {
    allowed = await Promise.all(modes.map(mode => {
      return acl.can(userId, mode, request) 
    }))
  } catch (error) {
    console.log({error})
  }
  ...
  if (!allAllowed) {
    const errors = await Promise.all(modes.map(mode => acl.getError(userId, mode, request)))
