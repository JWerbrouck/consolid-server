...
const extractCertificate = require('../consolid/helper')
...
function allow (mode, checkPermissionsForDirectory) {
  return async function allowHandler (req, res, next) {
    ...
    // TO DO: extract formdata and control for certificates
    // certificates must be in RDF, stored in an Array
    // otherwise: null
    let formData, np_certs
    try {
      formData = await extractCertificate(req)
      if ('certificates' in formData) {
        np_certs = formData.certificates
      } else {
        np_certs = false
      }
    } catch (error) {
    ...
    const isAllowed = await req.acl.can(userId, mode, np_certs)
    if (isAllowed) {
      return next()
    }
    if (mode === 'Read' && (resourcePath === '' || resourcePath === '/')) {
      ...
      if (representationPath) {
        ...
        const representationIsAllowed = await req.acl.can(userId, mode, np_certs)
