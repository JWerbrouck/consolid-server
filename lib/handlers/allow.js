...
function allow (mode, checkPermissionsForDirectory) {
  return async function allowHandler (req, res, next) {
    ...
    const isAllowed = await req.acl.can(userId, mode, req, res)
    if (isAllowed) {
      return next()
    }
    if (mode === 'Read' && (resourcePath === '' || resourcePath === '/')) {
      ...
      if (representationPath) {
        ...
        const representationIsAllowed = await req.acl.can(userId, mode, req, res)
        if (representationIsAllowed) {
          return next()
        }
      }
    }
    const error = req.authError || req.acl.getError(userId, mode, req)
    debug(`${mode} access denied to ${userId || '(none)'}: ${error.status} - ${error.message}`)
    console.log(`${mode} access denied to ${userId || '(none)'}: ${error.status} - ${error.message}`)
    next(error)
  }
}
