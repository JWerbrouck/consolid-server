...
function extractCertificatesFromText(certs) {
    let certificates = {}
    try {
        if (Array.isArray(certs)) {
            for (const cert of certs) {
                const { total, prefixes, Head, assertion, provenance, pubinfo, certUri } = decomposeNP(cert)
                certificates[certUri] = { total, prefixes, Head, assertion, provenance, pubinfo }
            }
            return certificates
        } else {
            const { total, prefixes, Head, assertion, provenance, pubinfo, certUri } = decomposeNP(certs)
            certificates[certUri] = { total, prefixes, Head, assertion, provenance, pubinfo }
            return certificates
        }
    } catch (error) {
    ...
    async function fetchNP(url) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const { total, prefixes, Head, assertion, provenance, pubinfo, certUri } = decomposeNP(rawDoc)
...
async function extractFromFormData(req) {
    return new Promise(async (resolve, reject) => {
        try {
